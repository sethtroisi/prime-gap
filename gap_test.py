...
def get_arg_parser():
    ...
    parser.add_argument('--mstart',  type=int, required=True)
    parser.add_argument('--mint',    type=int, required=True)
    ...
    parser.add_argument('--sieve-range',  type=int, required=True,
...
def determine_next_prime_i(m, K, composites):
    center = m * K
    tests = 0
...
def determine_prev_prime_i(m, K, composites):
    center = m * K
    tests = 0
...
def prime_gap_test(args):
    ...
    for mi in range(M_inc):
        ...
        if run_prp:
            tests, prev_p_i = determine_next_prime_i(m, K, composite[0])
            s_total_prp_tests += tests
            s_gap_out_of_sieve_prev += prev_p_i < composite[0][-1]

            tests, next_p_i = determine_next_prime_i(m, K, composite[1])
            s_total_prp_tests += tests
            s_gap_out_of_sieve_next += next_p_i > composite[1][-1]
