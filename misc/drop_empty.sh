...
FROM range
WHERE num_processed = 0 AND time_tests <= 1
    AND 0 = (SELECT COUNT(*) FROM m_stats m WHERE m.rid = range.rid AND (
                 test_time > 0 OR next_p != 0 OR prev_p != 0 OR prp_next + prp_prev > 0));
EOL
