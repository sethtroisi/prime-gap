...
sqlite3 gaps.db -header -separator $'\t' 'SELECT year,COUNT(*),ROUND(AVG(merit),1) FROM gaps WHERE year > 2010 GROUP BY 1'
...
MANY="$(sqlite3 gaps.db 'SELECT startprime FROM gaps' | sed -n 's!^[^/]*[^0-9/]\([0-9]\+#\).*!\1!p' | sort | uniq -c | sort -nr | awk '$1 > 500' | sed 's/#//')"
...
for P in $(echo "$MANY" | head -n 15 | awk '{print $2}'); do
